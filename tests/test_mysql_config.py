...
class MysqlConfigTest(unittest.TestCase):
    ...
    def test_client_entrypoints_live_in_client_package_without_root_compatibility_shells(self):
        ...
        try:
            ...
            mac_build_script = Path("client/build_mac.sh").read_text(encoding="utf-8")
            ...
            self.assertIn("python -m pip install", build_script)
            self.assertIn("python -m PyInstaller", build_script)
            self.assertIn("client-runtime-requirements.txt", build_script)
            self.assertIn("echo requests", build_script)
            self.assertIn("echo pywebview", build_script)
            self.assertIn("BUILD_LOG=build\\client-build.log", build_script)
            self.assertIn("BUILD_RESULT=dist\\BUILD_RESULT.txt", build_script)
            self.assertIn("BUILD_STATUS=FAILED", build_script)
            self.assertIn("BUILD_STATUS=SUCCESS", build_script)
            self.assertIn("type dist\\BUILD_RESULT.txt", build_script)
            self.assertIn("Build result saved to dist\\BUILD_RESULT.txt", build_script)
            self.assertNotIn("eth-account", build_script)
            self.assertIn("PyInstaller is required", build_script)
            self.assertIn("https://pypi.tuna.tsinghua.edu.cn/simple", build_script)
            self.assertIn("if errorlevel 1", build_script)
            self.assertTrue(all(ord(ch) < 128 for ch in build_script))
            self.assertIn("set -e", mac_build_script)
            self.assertIn("python3 -m pip install", mac_build_script)
            self.assertIn("python3 -m PyInstaller", mac_build_script)
            self.assertIn("client-runtime-requirements.txt", mac_build_script)
            self.assertIn("requests", mac_build_script)
            self.assertIn("pywebview", mac_build_script)
            self.assertIn("client/main.py", mac_build_script)
            self.assertIn("--name web3-node", mac_build_script)
            self.assertIn("dist/node_config.json", mac_build_script)
            self.assertIn("dist/.env", mac_build_script)
            self.assertIn("pypi.tuna.tsinghua.edu.cn/simple", mac_build_script)
            self.assertNotIn("-r requirements.txt", mac_build_script)
            self.assertNotIn("install -r requirements.txt", mac_build_script)
            self.assertNotIn("client/node_mac.py", mac_build_script)
            self.assertNotIn("eth-account", mac_build_script)
            self.assertTrue(all(ord(ch) < 128 for ch in mac_build_script))
